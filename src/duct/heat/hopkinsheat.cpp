...
namespace duct{
  ...
  HopkinsHeatSource::HopkinsHeatSource(const LaminarDuct& t):
    cshape(t.geom().shape())
  {
    TRACE(10,"HopkinsHeatSource::HopKinsHeatSource(Duct)");
    this->t=&t;			// Save pointer to duct
    setZeroFreq(cshape);
  ...
  void HopkinsHeatSource::setZeroFreq(const string& shape){
    ...
    if(shape.compare("vert")==0){
      ...
      {
        WARN("Warning: duct.geom.shape unknown for ZeroHeatH. Aborting...");
        abort();
      }
  }
  ...
  vd HopkinsHeatSource::Qsf(const Cell& v) const{
    ...
    vd heat=-v.vSf*(H*v.T()());
    heat(0)+=v.vSf*(H(0,0)*v.Tw()(0)); // Time-averaged component
				       // subtracted
