...
class segment:
	...
	def idft1long(self,  var):
		cdef double Nf = self.Nf
		cdef double Ns = self.Ns
		newvar = n.zeros((1,self.Ns), complex)
		newvar[0,0] = var[0,0] * Ns
	...
	def complextofull(self,complexval):
		...
		return full
	def fulltocomplex(self,n.ndarray[dtype=n.float_t,ndim=1] full):
		cdef unsigned int length=full.shape[0]/2
