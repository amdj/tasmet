...
class tube_constantcs(segment,testing,testbc_constantcs):
...
# 			#=rho[0].ravel()
	...
	def innerError(self,error):
		...
		for i in xrange(1,self.gridpoints-1):
			...
			drhoudx=(self.mf()[i+1]-self.mf()[i-1])/(self.x[i+1]-self.x[i-1])
			drhoudxlin=self.rhom*(self.U()[i+1]-self.U()[i-1])/(self.x[i+1]-self.x[i-1])
			error[rhoblk[0]:rhoblk[1]]=dmdt+drhoudxlin

			#Implementation of momentum equation
			dmfdt=1j*self.omega*self.mf()[i]
			dmfdtlin=self.rhom*1j*self.omega*self.U()[i]
			dpdx=(self.p()[i+1]-self.p()[i-1])/(self.x[i+1]-self.x[i-1])
			rhouudotn=0.5*(self.momf()[i+1]-self.momf()[i-1])
			#FIXTHIS
			error[Ublk[0]:Ublk[1]]=dmfdtlin+dpdx#+rhouudotn
