...
class var {
	public:
		...
		virtual ~var();
		const us Ns,Nf;
		const varoperations& vop; //Get a reference to the class container, which should have an instance available of varoperations.
	protected:
...
class vvar { //variable container, including some methods to get and set the data simultaneously
	...
	protected:
		...
		us Dofs,Ns,Nf,gp; //Degrees of freedom in variable, Number of time samples, Number of frequencies, number of gridpoints
