...
{
	TRACELOG("operator*(const var& var1,const var& var1) const");
	TRACELOG("var1.tdata:" << var1.tdata());
	TRACELOG("var2.tdata:" << var2.tdata());
	vd tdata=var1.tdata()%var2.tdata();
	var newvar(var1.vop);
	newvar.settdata(tdata);
	TRACELOG("newvar.tdata:" << newvar.tdata());
	return newvar;
}
...
var::var(const varoperations& vop,double initval) :vop(vop),Nf(vop.Nf),Ns(vop.Ns) {
	TRACELOG("var::var(const varoperations& vop)");
    timedata=vd(Ns);
    amplitudedata=vd(Ns);
    settdata(initval);
}
...
void var::settdata(double val) {
	TRACELOG("var::settdata(double val)");
    timedata.fill(val);
    dft();
}
void var::settdata(vd& val) {
	TRACELOG("var::settdata(vd& val)");
	TRACELOG("val.size():"<<val.size());

	timedata=val;
	TRACELOG("timedata.size():"<<timedata.size());
	TRACELOG("vop.fDFT" << vop.fDFT);
    dft();
    TRACELOG("var::settdata(vd& val) done.");
}
//Show methods
void var::showtdata() {
    unsigned i;
...
var& var::operator=(const var& v){
	TRACELOG("var::operator=(const var& v)");
	this->timedata=v.timedata;
//	TRACELOG("this->timedata:" <<this->timedata);
//	TRACELOG("v.timedata:" <<v.timedata);
...
vvar::vvar(string name,us gp,us Nf,double initv) :
 Name(name),gp(gp),Nf(Nf) {
	...
	Dofs=Ns*gp;
	TRACELOG("Warning, vvar not changed for varoperations!!");
...
varoperations::varoperations(us Nf,d freq): Nf(Nf){
	TRACELOG("varoperations::varoperations(Nf,freq)");
	//ctor
	Ns=2*Nf+1;
//    cout << "fop.Ns:" << Ns << endl;
//    cout << "fop.Nf:" << Nf << endl;
...
void varoperations::updateiomg(){
	TRACELOG("varoperations::updateiomg()");
	for(us i=1;i<=Nf;i++){
